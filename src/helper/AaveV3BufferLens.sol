...
contract AaveV3BufferLens is IBufferLens {
    /// @notice Thrown when the queried asset is not a listed reserve in the helper's Aave V3 pool.
    error AaveV3BufferLens__ReserveNotListed(address asset);
        ...
        returns (uint256 withdrawableAmount)
    {
        ...
        if (address(withdrawBufferHelper) == address(0)) {
            ...
            // asset). Checked first so a misconfiguration fails loudly rather than being masked by the
            // runtime paused/inactive checks below (which also read 0 for an unlisted reserve), matching
            // AaveV4BufferLens.
            address aTokenAddress = IPool(aaveV3Pool).getReserveData(address(asset)).aTokenAddress;
            if (aTokenAddress == address(0)) revert AaveV3BufferLens__ReserveNotListed(address(asset));
