...
contract CbBtcUsdcMorphoBalanceAdapter {
    ...
    function cbBtcUsdPrice() public view returns (uint256) {
        return collateralUsdFeed.getPrice();
    }

    function usdcUsdPrice() public view returns (uint256) {
        return debtUsdFeed.getPrice();
    }

    /// @notice Returns net TVL of a user denominated in the base token.
    ...
    /// @notice Returns the full position breakdown for a user, all in base token units.
        ...
        returns (uint256 totalCollateralInCbBTC, uint256 totalDebtInCbBTC, uint256 totalCreditInCbBTC)
    {
        ...
        totalCollateralInCbBTC = _assetToBase(collateralInCollateralTokenAmount, collateralDecimals, collateralUsdFeed);
        totalCreditInCbBTC = _assetToBase(suppliedInDebtTokenAmount, loanDecimals, debtUsdFeed);
        totalDebtInCbBTC = _assetToBase(debtInDebtTokenAmount, loanDecimals, debtUsdFeed);
    }
}
