...
//@notice Shadow Exchange's Centralized Liquidity Pool has customized some of the configs,
//    hence the need to have a separate interface for it. For example, no permitting functionality
//    so no nonce and no operator; and PoolKey is determined by token0, token1, and tickSpacing instead
//    of fee.
interface INonFungiblePositionManagerShadow {
    struct Position {
        // the ID of the pool with which this token is connected
        uint80 poolId;
        // the tick range of the position
