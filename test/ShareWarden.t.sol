...
contract ShareWardenTest is Test, MerkleTreeHelper {
    ...
    function testDepositWithMultipleUsers() external {
        ...
        vm.startPrank(user1);
        WETH.safeApprove(address(boringVault), depositAmount);
        uint256 shares1 = teller.deposit(DepositParams(WETH, depositAmount, 0, address(0)), referrer, ComplianceData(0, ""));
        vm.stopPrank();
        ...
        vm.startPrank(user2);
        WETH.safeApprove(address(boringVault), depositAmount);
        uint256 shares2 = teller.deposit(DepositParams(WETH, depositAmount, 0, address(0)), referrer, ComplianceData(0, ""));
        vm.stopPrank();
    ...
    function testBatchUpdateBlacklistMultipleAddresses() external {
        ...
        vm.startPrank(user3);
        WETH.safeApprove(address(boringVault), depositAmount);
        uint256 shares3 = teller.deposit(DepositParams(WETH, depositAmount, 0, address(0)), referrer, ComplianceData(0, ""));
        vm.stopPrank();
    ...
    function testBeforeTransferWithSingleParameter() external {
        ...
        vm.startPrank(user1);
        WETH.safeApprove(address(boringVault), depositAmount);
        teller.deposit(DepositParams(WETH, depositAmount, 0, address(0)), referrer, ComplianceData(0, ""));
        vm.stopPrank();
    ...
    function testFuzzDepositAndTransferWithShareWarden(uint256 amount) external {
        ...
        uint256 shares = teller.deposit(DepositParams(WETH, amount, 0, address(0)), referrer, ComplianceData(0, ""));
        vm.stopPrank();
