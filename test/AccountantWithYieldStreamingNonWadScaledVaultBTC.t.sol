...
contract AccountantWithYieldStreamingTest is Test, MerkleTreeHelper {
    ...
    function setUp() external {
        ...
        teller = new TellerWithYieldStreaming(address(this), address(boringVault), address(accountant), address(WBTC));
    ...
    function testPerformanceFeesAfterYield() external {
        ...
        assertApproxEqAbs(
            actualFees - platformFee,
            expectedPerformanceFees,
            1e1,
            "Performance fees should match share price appreciation"
        );
    ...
    function testFuzzDepositsWithNoYield(uint96 WBTCAmount0, uint96 WBTCAmount1) external {
        ...
        uint256 totalAssetsAfter = accountant.totalAssets();
        assertApproxEqRel(
            totalAssetsAfter, uint256(WBTCAmount0) + uint256(WBTCAmount1), toleranceTotal, "total assets mismatch"
        );
    ...
    function testFuzzDepositsWithYield(uint96 WBTCAmount0, uint96 WBTCAmount1, uint96 yieldVestAmount) external {
        ...
        uint256 expectedTotalAssets =
            (shares0 + expectedShares1).mulDivDown(sharePrice, 1e8) + accountant.getPendingVestingGains(); //verify the total assets == amount of shares
    ...
    function testFuzzWithdrawWithYield(uint96 WBTCAmount0, uint96 WBTCAmount1, uint96 yieldVestAmount) external {
        ...
        assertApproxEqRel(
            actualWithdrawn1, WBTCAmount1, ACCEPTABLE_PRECISION_LOSS, "Second depositor should get back their deposit"
        );
    ...
    function _deposit(uint256 amount, address user) internal returns (uint256) {
        ...
        uint256 shares = teller.deposit(DepositParams(WBTC, WBTCAmount, 0, address(0)), referrer, ComplianceData(0, ""));
        vm.stopPrank();
        return shares;
    }
