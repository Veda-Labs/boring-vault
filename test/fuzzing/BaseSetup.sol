...
contract BaseSetup is Test {
    ...
    bytes4 internal constant DEPOSIT_SELECTOR = TellerWithMultiAssetSupport.deposit.selector;
    ...
    function _deployRPSystem() internal {
        // Deploy RP vault
        vaultRP = new BoringVault(owner, "Boring Vault RP", "BVRP", 18);
        ...
        tellerMAS = new TellerWithMultiAssetSupport(owner, address(vaultRP), address(accountantRP), address(weth));
        tellerMAS.setAuthority(rolesAuthorityRP);
    }

    function _deployYSSystem() internal {
        // Deploy YS vault
        vaultYS = new BoringVault(owner, "Boring Vault YS", "BVYS", 18);
        ...
        tellerYS = new TellerWithYieldStreaming(owner, address(vaultYS), address(accountantYS), address(weth));
    ...
    function _setupRPRoles() internal {
        ...
        rolesAuthorityRP.setRoleCapability(
            MANAGER_ROLE, address(vaultRP), bytes4(keccak256("manage(address,bytes,uint256)")), true
        );
        rolesAuthorityRP.setRoleCapability(
            MANAGER_ROLE, address(vaultRP), bytes4(keccak256("manage(address[],bytes[],uint256[])")), true
        );
        rolesAuthorityRP.setRoleCapability(
            OWNER_ROLE, address(vaultRP), BoringVault.setBeforeTransferHook.selector, true
        );

        // AccountantRP capabilities
        rolesAuthorityRP.setRoleCapability(
            ADMIN_ROLE, address(accountantRP), AccountantWithRateProviders.pause.selector, true
        );
        rolesAuthorityRP.setRoleCapability(
            ADMIN_ROLE, address(accountantRP), AccountantWithRateProviders.unpause.selector, true
        );
        rolesAuthorityRP.setRoleCapability(
            ADMIN_ROLE, address(accountantRP), AccountantWithRateProviders.updateDelay.selector, true
        );
        rolesAuthorityRP.setRoleCapability(
            ADMIN_ROLE, address(accountantRP), AccountantWithRateProviders.updateUpper.selector, true
        );
        rolesAuthorityRP.setRoleCapability(
            ADMIN_ROLE, address(accountantRP), AccountantWithRateProviders.updateLower.selector, true
        );
        rolesAuthorityRP.setRoleCapability(
            ADMIN_ROLE, address(accountantRP), AccountantWithRateProviders.updatePlatformFee.selector, true
        );
        rolesAuthorityRP.setRoleCapability(
            ADMIN_ROLE, address(accountantRP), AccountantWithRateProviders.updatePerformanceFee.selector, true
        );
        rolesAuthorityRP.setRoleCapability(
            ADMIN_ROLE, address(accountantRP), AccountantWithRateProviders.updatePayoutAddress.selector, true
        );
        rolesAuthorityRP.setRoleCapability(
            ADMIN_ROLE, address(accountantRP), AccountantWithRateProviders.setRateProviderData.selector, true
        );
        rolesAuthorityRP.setRoleCapability(
            ADMIN_ROLE, address(accountantRP), AccountantWithRateProviders.resetHighwaterMark.selector, true
        );
        rolesAuthorityRP.setRoleCapability(
        ...
        rolesAuthorityRP.setRoleCapability(
            MINTER_ROLE, address(accountantRP), AccountantWithRateProviders.claimFees.selector, true
        );

        // TellerMAS capabilities
        rolesAuthorityRP.setRoleCapability(
            ADMIN_ROLE, address(tellerMAS), TellerWithMultiAssetSupport.pause.selector, true
        );
        rolesAuthorityRP.setRoleCapability(
            ADMIN_ROLE, address(tellerMAS), TellerWithMultiAssetSupport.unpause.selector, true
        );
        rolesAuthorityRP.setRoleCapability(
            ADMIN_ROLE, address(tellerMAS), TellerWithMultiAssetSupport.updateAssetData.selector, true
        );
        rolesAuthorityRP.setRoleCapability(
            ADMIN_ROLE, address(tellerMAS), TellerWithMultiAssetSupport.setShareLockPeriod.selector, true
        );
        rolesAuthorityRP.setRoleCapability(
        ...
        rolesAuthorityRP.setRoleCapability(
            SOLVER_ROLE, address(tellerMAS), TellerWithMultiAssetSupport.bulkDeposit.selector, true
        );
        rolesAuthorityRP.setRoleCapability(
            SOLVER_ROLE, address(tellerMAS), TellerWithMultiAssetSupport.bulkWithdraw.selector, true
        );
        rolesAuthorityRP.setPublicCapability(address(tellerMAS), DEPOSIT_SELECTOR, true);
        rolesAuthorityRP.setPublicCapability(
            address(tellerMAS), TellerWithMultiAssetSupport.depositWithPermit.selector, true
        );
    ...
    function _setupYSRoles() internal {
        ...
        rolesAuthorityYS.setRoleCapability(
            MANAGER_ROLE, address(vaultYS), bytes4(keccak256("manage(address,bytes,uint256)")), true
        );
        rolesAuthorityYS.setRoleCapability(
            MANAGER_ROLE, address(vaultYS), bytes4(keccak256("manage(address[],bytes[],uint256[])")), true
        );
        rolesAuthorityYS.setRoleCapability(
            OWNER_ROLE, address(vaultYS), BoringVault.setBeforeTransferHook.selector, true
        );

        // AccountantYS capabilities
        rolesAuthorityYS.setRoleCapability(
            ADMIN_ROLE, address(accountantYS), AccountantWithRateProviders.pause.selector, true
        );
        rolesAuthorityYS.setRoleCapability(
            ADMIN_ROLE, address(accountantYS), AccountantWithRateProviders.unpause.selector, true
        );
        rolesAuthorityYS.setRoleCapability(
            ADMIN_ROLE, address(accountantYS), AccountantWithRateProviders.updateDelay.selector, true
        );
        rolesAuthorityYS.setRoleCapability(
            ADMIN_ROLE, address(accountantYS), AccountantWithRateProviders.updateUpper.selector, true
        );
        rolesAuthorityYS.setRoleCapability(
            ADMIN_ROLE, address(accountantYS), AccountantWithRateProviders.updateLower.selector, true
        );
        rolesAuthorityYS.setRoleCapability(
            ADMIN_ROLE, address(accountantYS), AccountantWithRateProviders.updatePlatformFee.selector, true
        );
        rolesAuthorityYS.setRoleCapability(
            ADMIN_ROLE, address(accountantYS), AccountantWithRateProviders.updatePerformanceFee.selector, true
        );
        rolesAuthorityYS.setRoleCapability(
            ADMIN_ROLE, address(accountantYS), AccountantWithRateProviders.updatePayoutAddress.selector, true
        );
        rolesAuthorityYS.setRoleCapability(
            ADMIN_ROLE, address(accountantYS), AccountantWithRateProviders.setRateProviderData.selector, true
        );
        rolesAuthorityYS.setRoleCapability(
            ADMIN_ROLE, address(accountantYS), AccountantWithRateProviders.resetHighwaterMark.selector, true
        );
        rolesAuthorityYS.setRoleCapability(
            STRATEGIST_ROLE, address(accountantYS), AccountantWithYieldStreaming.vestYield.selector, true
        );
        rolesAuthorityYS.setRoleCapability(
            STRATEGIST_ROLE, address(accountantYS), AccountantWithYieldStreaming.postLoss.selector, true
        );
        rolesAuthorityYS.setRoleCapability(
            STRATEGIST_ROLE, address(accountantYS), bytes4(keccak256("updateExchangeRate()")), true
        );
        rolesAuthorityYS.setRoleCapability(
            MINTER_ROLE, address(accountantYS), AccountantWithYieldStreaming.setFirstDepositTimestamp.selector, true
        );
        rolesAuthorityYS.setRoleCapability(
            MINTER_ROLE, address(accountantYS), AccountantWithRateProviders.claimFees.selector, true
        );

        // TellerYS capabilities
        rolesAuthorityYS.setRoleCapability(
            ADMIN_ROLE, address(tellerYS), TellerWithMultiAssetSupport.pause.selector, true
        );
        rolesAuthorityYS.setRoleCapability(
            ADMIN_ROLE, address(tellerYS), TellerWithMultiAssetSupport.unpause.selector, true
        );
        rolesAuthorityYS.setRoleCapability(
            ADMIN_ROLE, address(tellerYS), TellerWithMultiAssetSupport.updateAssetData.selector, true
        );
        rolesAuthorityYS.setRoleCapability(
            ADMIN_ROLE, address(tellerYS), TellerWithMultiAssetSupport.setShareLockPeriod.selector, true
        );
        rolesAuthorityYS.setRoleCapability(
        ...
        rolesAuthorityYS.setRoleCapability(
            SOLVER_ROLE, address(tellerYS), TellerWithMultiAssetSupport.bulkDeposit.selector, true
        );
        rolesAuthorityYS.setRoleCapability(
            SOLVER_ROLE, address(tellerYS), TellerWithMultiAssetSupport.bulkWithdraw.selector, true
        );
        rolesAuthorityYS.setPublicCapability(address(tellerYS), DEPOSIT_SELECTOR, true);
        rolesAuthorityYS.setPublicCapability(
            address(tellerYS), TellerWithMultiAssetSupport.depositWithPermit.selector, true
        );
    ...
    function _configureAssets() internal {
        ...
        for (uint256 i = 0; i < NUM_ALT_ASSETS; i++) {
            accountantRP.setRateProviderData(alternativeAssets[i], false, address(altAssetRateProviders[i]));
