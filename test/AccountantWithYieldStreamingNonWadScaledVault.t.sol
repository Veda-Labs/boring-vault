...
contract AccountantWithYieldStreamingTest is Test, MerkleTreeHelper {
    ...
    function setUp() external {
        ...
        teller = new TellerWithYieldStreaming(
            address(this), address(boringVault), address(accountant), getAddress(sourceChain, "USDC")
        );
    ...
    function testYieldStreamUpdateDuringExistingStream() external {
        ...
        (, uint128 gains, uint128 lastVestingUpdate, uint64 startVestingTime, uint64 endVestingTime) =
            accountant.vestingState();
    ...
    function testPerformanceFeesAfterYield() external {
        ...
        assertEq(
            actualFees - platformFee, expectedPerformanceFees, "Performance fees should match share price appreciation"
        );
    ...
    function testFuzzDepositsWithNoYield(uint96 USDCAmount0, uint96 USDCAmount1) external {
        ...
        uint256 totalAssetsAfter = accountant.totalAssets();
        assertApproxEqRel(
            totalAssetsAfter, uint256(USDCAmount0) + uint256(USDCAmount1), toleranceTotal, "total assets mismatch"
        );
    ...
    function testFuzzDepositsWithYield(uint96 USDCAmount0, uint96 USDCAmount1, uint96 yieldVestAmount) external {
        ...
        uint256 expectedTotalAssets =
            (shares0 + expectedShares1).mulDivDown(sharePrice, 1e6) + accountant.getPendingVestingGains(); //verify the total assets == amount of shares
    ...
    function testFuzzWithdrawWithYield(uint96 USDCAmount0, uint96 USDCAmount1, uint96 yieldVestAmount) external {
        ...
        assertLe(
            actualWithdrawn0,
            firstDepositorExpectedValue,
            "first deposit actual withdrawn > first depositor expected value"
        );
        ...
        assertApproxEqRel(
            actualWithdrawn1, USDCAmount1, ACCEPTABLE_PRECISION_LOSS, "Second depositor should get back their deposit"
        );
    ...
    function testRoundingIssuesAfterYieldStreamAlmostEndsMinorWeiVestFuzz(
        uint96 USDCAmount0,
        uint96 USDCAmount1,
        uint256 yieldAmount
    ) external {
     ...
     */
    function testFuzzMultipleYieldVestsInvariant(uint96 depositAmount, uint96 yield1, uint96 yield2, uint96 yield3)
        external
    {
    ...
    function _assertExchangeRateVsVirtualSharePrice() internal view {
        ...
        assertLt(
            difference, MAX_PRECISION_LOSS_RAY, "lastVirtualSharePrice is too much higher than scaled exchangeRate"
        );
    }
    ...
    function _deposit(uint256 amount, address user) internal returns (uint256) {
        ...
        uint256 shares = teller.deposit(DepositParams(USDC, USDCAmount, 0, address(0)), referrer, ComplianceData(0, ""));
        vm.stopPrank();
        return shares;
    }
