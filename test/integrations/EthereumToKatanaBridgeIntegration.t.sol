...
contract KatanaOVaultIntegrationTest is Test, MerkleTreeHelper {
    ...
    function setUp() external {
        ...
        rolesAuthority.setRoleCapability(
            MANAGER_ROLE, address(boringVault),
        ...
        rolesAuthority.setRoleCapability(
            MANGER_INTERNAL_ROLE, address(manager),
            ManagerWithMerkleVerification.manageVaultWithMerkleVerification.selector, true
        );
        rolesAuthority.setRoleCapability(
            ADMIN_ROLE, address(manager),
            ManagerWithMerkleVerification.setManageRoot.selector, true
        );
        rolesAuthority.setRoleCapability(
            BORING_VAULT_ROLE, address(manager),
            ManagerWithMerkleVerification.flashLoan.selector, true
        );
        rolesAuthority.setRoleCapability(
            BALANCER_VAULT_ROLE, address(manager),
            ManagerWithMerkleVerification.receiveFlashLoan.selector, true
        );
        ...
        rolesAuthority.setUserRole(
            getAddress(sourceChain, "vault"), BALANCER_VAULT_ROLE, true
        );
        ...
        )
    {
        ...
        (uint256 expectedShares, uint256 minShares) = _previewVaultDeposit(
            amountToDeposit,
            slippageBps
        );
    ...
    // ────────────────────────────────────────────────────────────────────────
    function _previewVaultDeposit(
        uint256 assetsToDeposit,
        uint256 slippageBps
    ) internal view returns (uint256 expectedShares, uint256 minShares) {
        (bool ok, bytes memory data) = VBUSDC_VAULT_BRIDGE.staticcall(
            abi.encodeWithSignature("previewDeposit(uint256)", assetsToDeposit)
        );
