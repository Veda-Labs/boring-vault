...
contract BeraborrowIntegrationTest is BaseTestIntegration {
    ...
    function testBeraborrowIntegration() external {
        ...
        tx_.targetData[2] = abi.encodeWithSignature(
            "openDenVault((address,address,uint256,uint256,uint256,address,address,uint256,uint256,bytes))", openParams
        );
    ...
    function testBeraborrowManagedVaultIntegration() external {
        ...
        DecoderCustomTypes.AddCollParams memory addCollParams =
            DecoderCustomTypes.AddCollParams(address(0), address(0), 0, 0);
    ...
    function testBeraborrowManagedVaultIntegrationWithdrawFromEpoch() external {
        ...
        DecoderCustomTypes.ExternalRebalanceParams memory params =
            DecoderCustomTypes.ExternalRebalanceParams(getAddress(sourceChain, "collVaultUnwrapper"), "", 0);

        tx_.targetData[0] = abi.encodeWithSignature(
            "withdrawFromEpoch(uint256,address,(address,bytes,uint256))", 10e8, address(boringVault), params
        );
