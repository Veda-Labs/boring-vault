...
contract ScrollBridgeIntegrationTest is Test, MerkleTreeHelper {
    ...
    function testBridgingToMainnetERC20() external {
        ...
        address gateway =
            IScrollGateway(getAddress(sourceChain, "scrollGatewayRouter")).getERC20Gateway(address(localTokens[0]));

        bytes[] memory targetData = new bytes[](2);
        targetData[0] = abi.encodeWithSignature("approve(address,uint256)", gateway, 100e6);
        targetData[1] = abi.encodeWithSignature(
