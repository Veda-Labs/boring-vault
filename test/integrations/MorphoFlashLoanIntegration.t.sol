...
contract MorphoFlashLoanIntegrationTest is Test, MerkleTreeHelper {
    ...
    function test__MorphoFlashLoanAdapterTest() external {
        ...
        bytes memory userData;
        {
            ...
            userData = abi.encode(
                getAddress(sourceChain, "USDC"), emptyProofs, emptyDecoders, emptyTargets, emptyData, emptyValues
            );
        }

        {
            ...
            outerLeafs[0].argumentAddresses[0] = getAddress(sourceChain, "USDC");
            ...
            targetData[0] = abi.encodeWithSignature(
                "morphoFlashLoan(address,uint256,bytes)", getAddress(sourceChain, "USDC"), flashLoanAmount, userData
            );
            ...
            bytes32[][] memory manageProofs = _getProofsUsingTree(outerLeafs, manageTree);

            uint256 vaultUsdcBefore = ERC20(getAddress(sourceChain, "USDC")).balanceOf(address(boringVault));
            uint256 morphoUsdcBefore =
                ERC20(getAddress(sourceChain, "USDC")).balanceOf(getAddress(sourceChain, "morphoBlue"));

            manager.manageVaultWithMerkleVerification(manageProofs, decodersAndSanitizers, targets, targetData, values);

            uint256 vaultUsdcAfter = ERC20(getAddress(sourceChain, "USDC")).balanceOf(address(boringVault));
            uint256 morphoUsdcAfter =
                ERC20(getAddress(sourceChain, "USDC")).balanceOf(getAddress(sourceChain, "morphoBlue"));
            uint256 adapterUsdcAfter = ERC20(getAddress(sourceChain, "USDC")).balanceOf(address(flashLoanAdapter));

            assertEq(vaultUsdcAfter, 0, "Vault should have spent all USDC on repayment");
            assertEq(morphoUsdcAfter, morphoUsdcBefore, "Morpho USDC balance should be unchanged");
            assertEq(adapterUsdcAfter, 0, "Adapter should hold no USDC after flash loan");

            console.log("Flash loan executed successfully");
            console.log("Vault USDC before:", vaultUsdcBefore);
            console.log("Vault USDC after:", vaultUsdcAfter);
            console.log("Morpho USDC delta:", morphoUsdcAfter - morphoUsdcBefore);
        }
    }
    ...
    function test__MorphoFlashLoanAdapter__RevertsOnNestedFlashLoan() external {
        ...
        vm.expectRevert(MorphoFlashLoanAdapter.MorphoFlashLoanAdapter__FlashLoanAlreadyInProgress.selector);
        manager.manageVaultWithMerkleVerification(
            outerProofs, outerDecoders, outerTargets, outerTargetData, outerValues
        );
    }
        ...
        )
    {
        ...
        outerTargetData[0] =
            abi.encodeWithSignature("morphoFlashLoan(address,uint256,bytes)", token, uint256(100_000e6), userData);
        outerDecoders = new address[](1);
        outerDecoders[0] = rawDataDecoderAndSanitizer;
        outerValues = new uint256[](1);
    }

    function _buildInnerFlashLoanUserData(address token, bytes32[][] memory manageTree)
        internal
        view
        returns (bytes memory)
    {
        ...
        bytes32[][] memory innerProofs = _getProofsUsingTree(innerLeafs, manageTree);
        ...
        innerTargetData[0] = abi.encodeWithSignature(
            "morphoFlashLoan(address,uint256,bytes)", token, uint256(1e6), _emptyUserData(token)
        );
        address[] memory innerDecoders = new address[](1);
        innerDecoders[0] = rawDataDecoderAndSanitizer;

        return abi.encode(token, innerProofs, innerDecoders, innerTargets, innerTargetData, new uint256[](1));
    }

    function _emptyUserData(address token) internal pure returns (bytes memory) {
        bytes32[][] memory emptyProofs = new bytes32[][](0);
        address[] memory emptyDecoders = new address[](0);
        address[] memory emptyTargets = new address[](0);
        bytes[] memory emptyData = new bytes[](0);
        uint256[] memory emptyValues = new uint256[](0);
        return abi.encode(token, emptyProofs, emptyDecoders, emptyTargets, emptyData, emptyValues);
    }
