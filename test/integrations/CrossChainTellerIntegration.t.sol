...
contract FullTellerDecoderAndSanitizer is TellerDecoderAndSanitizer {}
...
contract CrossChainTellerIntegration is BaseTestIntegration {
    ...
    function testCrossChainDepositAndBridgePayETH() external {
        ...
        _addCrossChainTellerLeafs(
            leafs, getAddress(sourceChain, "eBTCTeller"), depositAssets, feeAssets, abi.encode(layerZeroBaseEndpointId)
        );
        ...
        tx_.targetData[0] = abi.encodeWithSignature(
            "approve(address,uint256)", getAddress(sourceChain, "eBTCTeller"), type(uint256).max
        );
        tx_.targetData[1] =
    ...
    function testCrossChainDepositAndBridgePayETH__RevertsLt20() external {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                TellerDecoderAndSanitizer.TellerDecoderAndSanitizer__BridgeWildCardLengthMustBe32Bytes.selector
            )
        );
        _submitManagerCall(manageProofs, tx_);
    }
