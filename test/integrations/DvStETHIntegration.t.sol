...
contract BalancerV3IntegrationTest is BaseTestIntegration {
    ...
    function testDvStETHIntegrationEmergencyWithdraw() external {
        ...
        tx_.targetData[0] =
            abi.encodeWithSignature("emergencyWithdraw(uint256[],uint256)", amounts, block.timestamp + 5);
    ...
    function testDvStETHIntegrationReverts() external {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(DvStETHDecoderAndSanitizer.DvStETHDecoderAndSanitizer__OnlyOneAmount.selector)
        );
        _submitManagerCall(manageProofs, tx_);
    }
