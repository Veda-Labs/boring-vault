...
import {
    ITBAaveDecoderAndSanitizer
} from "src/base/DecodersAndSanitizers/Protocols/ITB/aave/AaveDecoderAndSanitizer.sol";
import {
    ITBCorkDecoderAndSanitizer
} from "src/base/DecodersAndSanitizers/Protocols/ITB/cork/CorkDecoderAndSanitizer.sol";
import {
    ITBCurveAndConvexNoConfigDecoderAndSanitizer
} from "src/base/DecodersAndSanitizers/Protocols/ITB/curve_and_convex/CurveAndConvexNoConfigDecoderAndSanitizer.sol";
import {
    ITBEigenLayerDecoderAndSanitizer
} from "src/base/DecodersAndSanitizers/Protocols/ITB/eigen_layer/EigenLayerDecoderAndSanitizer.sol";
import {
    ITBGearboxDecoderAndSanitizer
} from "src/base/DecodersAndSanitizers/Protocols/ITB/gearbox/GearboxDecoderAndSanitizer.sol";
import {
    ITBKarakDecoderAndSanitizer
} from "src/base/DecodersAndSanitizers/Protocols/ITB/karak/KarakDecoderAndSanitizer.sol";
import {
    ITBReserveDecoderAndSanitizer
} from "src/base/DecodersAndSanitizers/Protocols/ITB/reserve/ReserveDecoderAndSanitizer.sol";
import {
    ITBReserveERC20WrappedDecoderAndSanitizer
} from "src/base/DecodersAndSanitizers/Protocols/ITB/reserve/ReserveERC20WrappedDecoderAndSanitizer.sol";
import {
    ITBSyrupDecoderAndSanitizer
} from "src/base/DecodersAndSanitizers/Protocols/ITB/syrup/SyrupDecoderAndSanitizer.sol";
...
contract GigaDeployDecoderAndSanitizerScript is Script, ContractNames, MainnetAddresses, MerkleTreeHelper {
    ...
    function deployContract(string memory name, bytes memory creationCode, uint256 value)
        internal
        returns (address _contract)
    {
        ...
        _addTx(
            address(deployer),
            abi.encodeWithSelector(deployer.deployContract.selector, name, creationCode, constructorArgs, value),
            value
        );
    ...
    function deployContract(string memory name, bytes memory creationCode, bytes memory constructorArgs, uint256 value)
        internal
        returns (address _contract)
    {
        _contract = deployer.getAddress(name);
