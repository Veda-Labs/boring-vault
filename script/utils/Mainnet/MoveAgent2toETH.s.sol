...
/*
 * source .env && forge script script/utils/Mainnet/MoveAgent2toETH.s.sol:MoveFunds --fork-url mainnet --broadcast -vvvvv
 */
contract MoveFunds is Script, MerkleTreeHelper {
    ...
    address constant TARGET_AGENT_VAULT = 0x951f36b2F8Fd8B213AE999E53dF1c77749A6cDed;
    address constant TARGET_AGENT_TELLER = 0xB31d657fe51edAd5577af4D948c119C9895Ea757;
    ...
    function run() external {
        ...
        uint256 amount = ERC20(TARGET_AGENT_VAULT).balanceOf(CLIENT_VAULT);
        ...
        targetData[1] = abi.encodeWithSignature(
            ...
            WETH,
            amount, // 0.02498035255173558ß WETH
            0,
            CLIENT_VAULT
        );
        ...
        bytes memory packedWithdraw = abi.encodePacked(WETH, CLIENT_VAULT);
        bytes32 withdrawDigest = computeLeafDigest(BASEDECODER, targets[1], false, 0x3e64ce99, packedWithdraw);

        bytes32[][] memory manageProofs = new bytes32[][](2);
        manageProofs[0] = getMerkleProof(json, approveDigest);
        manageProofs[1] = getMerkleProof(json, withdrawDigest);
