...
/**
 *  source .env && forge script script/utils/Mainnet/CheckRoles.s.sol:Deploy --fork-url  $MAINNET_RPC_URL --verify --slow  -vvvv
 */
contract Deploy is Script {
    ...
    address public testAddress = 0xdDEbf1BCC0597415089475c78125E2A6ec481b1C;
        ...
        for (uint8 i = 0; i < rolesAuthority.length; i++) {
            ...
            for (uint16 y = 0; y < 256; y++) {
                if ((uint256(userRoles) >> y) & 1 != 0) {
                    console.log("has role", y, "from", testAddress);
